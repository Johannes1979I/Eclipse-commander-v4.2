...
class EclipseCommander {
    ...
    async initialize() {
        ...
        try {
            ...
            solarTracker.initialize();  // ✅ Solar Tracker
    ...
    setupModeHandlers() {
        ...
        // ✅ NUOVO: Event listener per eclisse selezionata → attiva solar tracker
        window.addEventListener('eclipse:selected', (e) => {
            const eclipse = e.detail;  // ✅ CORRETTO: detail contiene direttamente l'eclisse
            const location = locationManager.getCurrentLocation();
            
            console.log('=== ECLIPSE SELECTED EVENT ===');
            console.log('Eclipse:', eclipse);
            console.log('Location:', location);
            
            if (eclipse && location) {
                Utils.log(`Solar Tracker: aggiorno con eclisse ${eclipse.name}`);
                solarTracker.updateEclipse(eclipse, location);
            } else {
                console.log('❌ Eclipse o location mancante');
                if (!eclipse) console.log('   Eclipse è null/undefined');
                if (!location) console.log('   Location è null/undefined');
            }
        });
        ...
        window.addEventListener('mode:changed', (e) => {
            ...
            // Mostra div eclipse mode (unica modalità rimasta con UI)
